        ...
        for (int i4 = 0; i4 < 4; ++i4) { //4
            for (int i5 = 0; i5 < 2; ++i5) { // 2
                ...
                sprintf(string_f, "%s %s %s %s %s %s %s", flags[i1], flags[i2],
                        flags_wp[i5], files[i3], files[i4], flags_wp[i6], files[i4]);
                if (string_f[1] == '\0') string_f[0] = '\0';
                sprintf(string_greap,
                        "grep %s > grep_out_log.txt", string_f);
                sprintf(string_greap_cat,
                        "./s21_grep %s > s21_grep_out_log.txt",
                        string_f);
                system(string_greap);
